...
2. segment_ids 编码 sample_id * num_slots + triplet_type，其中 triplet_type: 0=anchor, 1=positive, 2+=negative
...
使用方法:
    ...
    for batch in pipeline.pack().load().build():
        ...
        segment_ids = batch["segment_ids"]  # [seq_length] - 编码 sample_id * num_slots + triplet_type
    ...
    def __call__(self, example: dict) -> dict | None:
        ...
        result = {
            ...
            "triplet_type": np.array(
                triplet_type, dtype=np.int32
            ),  # Internal use only, for computing segment_ids
        }
...
def create_dataset_pipeline(
    ...
    """Create a Pipeline for triplet data with segment_ids encoding sample_id * num_slots + triplet_type.
    ...
    for name, source in data.items():
        ...
        transformed_source = TransformedShardedSource(
            ...
            transform=ContrastiveInputTransform(
                tokenizer=tokenizer,
                num_negative_samples_per_anchor=num_negatives,
    ...
    def pack_with_custom_source(config=None):
    ...
    pipeline.pack = pack_with_custom_source
...
def _extract_single_sequence_info(
    ...
    num_slots: int,  # Required: Number of slots per sample (2 + num_negatives)
) -> tuple[list[int], list[int]]:
    """
    Vectorized extraction for a SINGLE 1D sequence based on SEGMENT IDs.

    Extracts sample_id from segment_ids and detects sample boundaries based on sample_id changes.
    segment_ids encodes sample_id * num_slots + triplet_type.

    Args:
        segment_ids: Segment IDs array [seq_len], encoding sample_id * num_slots + triplet_type
        attention_mask: Attention mask array [seq_len] (optional)
        num_slots: Number of slots per sample (2 + num_negatives). Required.

    Returns:
        Tuple of (start_indices, lengths) where:
        - start_indices: List of sample start positions
    ...
    # 2. 从 segment_ids 中提取 sample_id
    # segment_ids 编码为: sample_id * num_slots + triplet_type
    # 对于 padding (-1)，sample_id 也会是负数，需要过滤
    sample_ids = segment_ids // num_slots
    # 过滤掉 padding 位置（segment_ids < 0）
    valid_mask_for_sample = segment_ids >= 0
    # 对于无效位置，使用 -1 来标记
    sample_ids = np.where(valid_mask_for_sample, sample_ids, -1)

    # 3. 检测 sample_id 的变化来确定样本边界
    # 索引 0 永远是一个样本的开始（如果有效）
    if valid_length > 0 and sample_ids[0] >= 0:
        start_indices = [0]

        # 检测 sample_id 的变化
        # 只有当 sample_id 发生变化且新 sample_id 有效时，才是新样本的开始
        for i in range(1, valid_length):
            prev_sample_id = sample_ids[i - 1]
            curr_sample_id = sample_ids[i]

            # 如果当前 sample_id 有效，且与前一个不同，则是新样本的开始
            if curr_sample_id >= 0 and curr_sample_id != prev_sample_id:
    ...
    return start_indices, lengths.tolist()
...
def extract_sample_info_from_segment_ids(
    ...
    """
    Strictly extracts sample info from segment_ids.

    This function extracts sample boundaries by:
    1. Computing sample_id = segment_ids // num_slots
    2. Detecting changes in sample_id to identify sample boundaries

    Args:
        segment_ids: Segment IDs array, encoding sample_id * num_slots + triplet_type.
                     Must be 1D [seq_len] or 2D [batch_size, seq_len].
        num_slots: Number of slots per sample (2 + num_negatives). Required.
                   Each sample has: Anchor(1) + Positive(1) + Negatives(N) slots.
        attention_mask: Attention mask array (optional). If provided, must have the same shape as segment_ids.

    Returns:
        DataSampleInfo with sample boundaries extracted.

    Rules:
    1. segment_ids must be 1D or 2D.
    2. If attention_mask is provided, it MUST have the exact same shape as segment_ids.
    3. num_slots must be positive.
    """
    # 1. 统一转换为 Numpy Array
    ...
    if ndim == 1:
        ...
        starts, lengths = _extract_single_sequence_info(
            segment_ids_np, attention_mask_np, num_slots
        )
        all_starts = starts
        all_lengths = lengths
...
def extract_sample_info(
    ...
    num_negatives: int = 1,  # Number of negative samples per anchor-positive pair. Default: 1.
) -> DataSampleInfo:
    """
    Wrapper for dictionary batches.

    Args:
        batch: Dictionary containing segment_ids and optionally attention_mask
        max_samples: Maximum number of samples (optional, for validation)
        num_negatives: Number of negative samples per anchor-positive pair. Default: 1.

    Returns:
        DataSampleInfo with sample boundaries extracted from segment_ids.
    """
    ...
    info = extract_sample_info_from_segment_ids(segment_ids, num_slots, attention_mask)
...
def split_packed_sequence(
    ...
    Args:
        ...
        sample_info: Pre-computed sample info (optional)
        num_negatives: Number of negative samples per anchor-positive pair. Required if sample_info is None.
    ...
    if sample_info is None:
        ...
        # Calculate num_slots: Anchor(1) + Positive(1) + Negatives(N)
        num_slots = 2 + num_negatives
        sample_info = extract_sample_info_from_segment_ids(segment_ids, num_slots, None)
